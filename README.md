...
![Security Audit](https://img.shields.io/github/actions/workflow/status/kohld/cart-api/security-audit.yml?branch=main&style=flat-square&label=Security+Audit)
...
Instead, the API is documented via a dedicated flow guide and an Insomnia export:

- **Cart Flow:** step-by-step guide (register → login → cart) at [`docs/CART_FLOW.md`](docs/CART_FLOW.md)
- **Insomnia Export:** a ready-to-import collection is available at [`docs/insomnia.yaml`](docs/insomnia.yaml) for local testing
